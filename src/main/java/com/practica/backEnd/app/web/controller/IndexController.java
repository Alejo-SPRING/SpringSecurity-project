...
public class IndexController {
	...
	public String springLogin(@RequestParam(value="error", required = false) String error, @RequestParam(name = "logout", required = false) String logout, Model model, Principal principal, RedirectAttributes redirect) {
		if(principal != null) {
	...
	public String registrar(@Valid UsuarioLogin usuarioLogin, BindingResult result, RedirectAttributes redirect, Model model) {
		if (result.hasErrors()) {
			model.addAttribute("titulo", "RegistrarValid");
			return "registrar";
		} else {
			usuarioDatoDTO = usuarioLogin.getUsuarioDatos();
			usuarioDatoDAO.create(usuarioDatoDTO);
			usuarioLoginDAO.create(usuarioLogin);
			redirect.addFlashAttribute("mensaje", "success(\"Registrado, bienvenido!\");");
		}
		return "redirect:/SpringApp/filter";
	}
